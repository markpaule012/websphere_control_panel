	...
	**/
	...
	if (mysqli_num_rows($result) > 0) {
		while($row = mysqli_fetch_assoc($result)) {
			...
			exec('WMIC /node:' . $row["m_name"] . ' /user:' . $row["m_user"] . ' /password:' . $row["m_pass"] . ' process call create "' . $row["p_machineLocation"] . '\bin\startServer.bat ' . $row["p_serverName"] . '"', $output);
			if (strpos($output[6], "0") !== false){
				echo "Starting server! Please wait for 10-15 seconds to transmit the start command.";
			}else{
